...
\begin{titlepage}
  ...
  \begin{tabular}{rl}
    ...
    \textbf{Link deploy:} & \url{https://3-d-building-builder.vercel.app/} \\
  \end{tabular}
  \vfill
  {\large \today\par}
\end{titlepage}
...
So với yêu cầu tối thiểu của đề bài, sản phẩm vẫn đáp ứng hướng \textbf{Option A -- AI 3D Scene Describer}: mỗi ảnh/view được AI sinh tiêu đề, mô tả, 3--5 điểm nổi bật và nội dung thuyết minh phù hợp ngành. Phần mở rộng của sản phẩm là ghép các mô tả này thành một tour guide có audio, manifest, Supabase persistence và trình phát tour toàn màn hình để dùng lại.
...
\begin{itemize}
  ...
  \item Tự copy các dự án đang có trong localStorage lên Supabase khi backend đã cấu hình.
...
\texttt{POST /api/tts/generate} & Render audio WAV bằng VieNeu-TTS, upload Supabase Storage nếu cấu hình, fallback artifacts local. \\
\texttt{POST /api/tour-assets/image} & Nhận ảnh đã nén từ frontend và upload lên Supabase Storage bucket \texttt{3D}. \\
...
\begin{enumerate}
  ...
  \item Frontend validate input, nén ảnh lớn trước khi gửi backend và upload ảnh lên Supabase Storage nếu backend đã cấu hình.
  ...
  \item Backend render giọng nữ bằng VieNeu-TTS, lưu file WAV lên Supabase Storage hoặc fallback artifacts, rồi trả URL audio.
...
\begin{itemize}
  ...
  \item \textbf{Storage}: localStorage fallback; Supabase Postgres lưu project payload; Supabase Storage bucket \texttt{3D} lưu ảnh upload và audio WAV; artifacts local là fallback khi thiếu cấu hình.
  \item \textbf{Deployment}: frontend deploy tại \url{https://3-d-building-builder.vercel.app/}; backend chạy local/ngrok hoặc container Python lâu dài qua \texttt{NEXT\_PUBLIC\_API\_URL}.
\end{itemize}
...
Backend/API xử lý request & Có FastAPI backend với endpoint mô tả ảnh, tạo tour, TTS, upload media và Supabase. \\
...
\noindent\fbox{%
  \begin{minipage}{0.94\linewidth}
    ...
    \begin{itemize}
      ...
      \item \texttt{highlights}: 3--5 ý
      \item \texttt{digitization\_tips}: các điểm cần chú ý khi số hóa 3D
    \end{itemize}
    Tone phù hợp ngành bất động sản, bán lẻ hoặc triển lãm.
  \end{minipage}
}
...
\noindent\fbox{%
  \begin{minipage}{0.94\linewidth}
    Từ danh sách các điểm dừng đã có \texttt{title}, \texttt{description} và \texttt{highlights},
    hãy viết lời dẫn hướng dẫn viên bằng tiếng Việt gồm:
    \begin{itemize}
      \item \texttt{intro}
      \item \texttt{narration} cho từng điểm
      \item \texttt{outro}
    \end{itemize}
...
\begin{itemize}
  ...
  \item Viết test cho API mock, provider runtime, TTS response, Supabase fallback và Supabase Storage upload.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{TTS và storage}: file WAV cần được lưu lại để tour có thể mở lại mà không render lại; bản hiện tại đã upload được lên Supabase Storage khi có key backend.
...
\midrule
Supabase hiện lưu toàn bộ project payload JSONB, ảnh/audio ở bucket public và chưa có user authentication. & Trung bình. Phù hợp demo nhưng chưa đủ multi-user production. & Thêm Supabase Auth, user/workspace id, RLS policy theo user, signed URL cho media private và tách bảng project/stops/audio nếu cần query/report nâng cao. \\
\bottomrule
\end{longtable}
...
\begin{itemize}
  ...
  \item Thêm signed URL hoặc private bucket cho ảnh/audio khi triển khai production.
...
\begin{itemize}
  ...
  \item Frontend deploy: \url{https://3-d-building-builder.vercel.app/}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Link deploy}: \url{https://3-d-building-builder.vercel.app/}; backend dùng URL cấu hình qua \texttt{NEXT\_PUBLIC\_API\_URL}.
